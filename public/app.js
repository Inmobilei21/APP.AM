...
function openClientFiscalArea(){
  ...
  document.body.appendChild(overlay);const content=overlay.querySelector(".client-fiscal-content"),render=tab=>{overlay.querySelectorAll("[data-fiscal-tab]").forEach(button=>button.classList.toggle("active",button.dataset.fiscalTab===tab));if(tab==="documents")content.innerHTML=`<div class="client-fiscal-section-title"><div><h2>Documentación</h2><p>Consulta tus facturas y el resto de documentación fiscal y contable.</p></div><button type="button" data-open-all-documents>Ver todos los documentos →</button></div><div class="client-fiscal-folders"><button type="button" data-open-all-documents><span>${clientDesktopIcon("documents")}</span><div><strong>Facturas</strong><small>Todas tus facturas disponibles</small></div><b>›</b></button><button type="button" data-open-all-documents><span>${clientDesktopIcon("file")}</span><div><strong>Contabilidad</strong><small>Balances, libros y cuentas anuales</small></div><b>›</b></button><button type="button" data-open-all-documents><span>${clientDesktopIcon("documents")}</span><div><strong>Otra documentación</strong><small>Escrituras, certificados y comunicaciones</small></div><b>›</b></button></div><section class="client-fiscal-recent"><h3>Documentos recientes</h3><div class="client-fiscal-document"><span>${clientDesktopIcon("file")}</span><div><strong>Facturas del periodo actual</strong><small>Asesoría Fiscal y Contable · Facturas</small></div><button type="button" data-open-all-documents>Ver</button></div><div class="client-fiscal-document"><span>${clientDesktopIcon("file")}</span><div><strong>Resumen contable</strong><small>Asesoría Fiscal y Contable · Contabilidad</small></div><button type="button" data-open-all-documents>Ver</button></div></section>`;else if(tab==="declarations")content.innerHTML=`<div class="client-fiscal-section-title"><div><h2>Declaraciones</h2><p>Accede a los modelos tributarios presentados y a sus justificantes.</p></div></div><div class="client-fiscal-empty">${clientDesktopIcon("file")}<strong>Tus declaraciones aparecerán aquí</strong><small>Podrás consultar y descargar cada modelo por ejercicio y periodo.</small></div>`;else if(tab==="information")content.innerHTML=`<div class="client-fiscal-section-title"><div><h2>Información general</h2><p>Datos principales de tu servicio fiscal y contable.</p></div></div><div class="client-fiscal-info-grid"><article><small>Cliente</small><strong>${escapeHtml(clientPreviewName)}</strong></article><article><small>Servicio</small><strong>Fiscal y Contable</strong></article><article><small>Estado</small><strong class="active">Contratado</strong></article><article><small>Asesoría</small><strong>Asesoría Molinero</strong></article></div>`;else content.innerHTML=`<div class="client-fiscal-section-title"><div><h2>Reclamaciones</h2><p>Consulta o inicia una comunicación relacionada con una gestión fiscal.</p></div></div><div class="client-fiscal-empty">${clientDesktopIcon("messages")}<strong>No tienes reclamaciones abiertas</strong><small>Cuando exista una comunicación aparecerá en este apartado.</small></div>`;content.querySelectorAll("[data-open-all-documents]").forEach(button=>button.addEventListener("click",openClientDocuments))};
