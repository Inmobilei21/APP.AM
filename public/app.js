...
async function renderClientFiscalInformation(content){
  const client=(await getAllClientMetadata()).find(item=>clientIdentity(item)===clientPreviewName)||{},contacts=client.contacts||[],partners=client.partners||[],registry=client.commercialRegistry||{},address=registry.registeredAddress||{};
  const primary=contacts.find(item=>item.primary)||contacts[0]||{},obligations=Object.keys(client.obligations||{}).filter(key=>client.obligations[key]).map(key=>\`Modelo \${key}\`).join(", ");
  content.innerHTML=\`<div class="client-fiscal-section-title"><div><h2>Información general</h2><p>Información registrada en tu ficha de cliente. Estos datos son solo de consulta.</p></div><span class="client-fiscal-readonly">Solo lectura</span></div><div class="client-fiscal-info-sections"><section><h3>Datos del cliente</h3><div class="client-fiscal-info-grid">\${clientFiscalField("Nombre o razón social",client.name||clientPreviewName,true)}\${clientFiscalField("DNI / CIF",client.cif)}\${clientFiscalField("Tipo de persona",client.personType==="fisica"?"Persona física":"Persona jurídica")}\${clientFiscalField("Periodicidad fiscal",client.periodicity==="mensual"?"Mensual":"Trimestral")}\${clientFiscalField("Marco de información financiera",client.financialReportingFramework,true)}</div></section><section><h3>Contacto</h3><div class="client-fiscal-info-grid">\${clientFiscalField("Persona de contacto",primary.name||client.representative)}\${clientFiscalField("Teléfono",primary.phone||client.phones)}\${clientFiscalField("Correo electrónico",primary.email||client.emails,true)}</div></section>\${partners.length?\`<section><h3>Socios y administración</h3><div class="client-fiscal-info-grid">\${clientFiscalField("Tipo de administración",client.administrationType)}\${clientFiscalField("Socios",partners.map(partner=>\`\${partner.name||"—"}\${partner.percentage!==null&&partner.percentage!==undefined&&partner.percentage!==""?\` (\${partner.percentage} %)\`:""}\`).join(" · "),true)}</div></section>\`:""}<section><h3>Datos fiscales y domicilio</h3><div class="client-fiscal-info-grid">\${clientFiscalField("Obligaciones fiscales",obligations,true)}\${clientFiscalField("Domicilio",address.address,true)}\${clientFiscalField("Código postal",address.postalCode)}\${clientFiscalField("Municipio",address.municipality)}\${clientFiscalField("Provincia",window.SPAIN_LOCATIONS?.[address.province]?.name||address.province)}</div></section></div>\`;
}
function openClientFiscalArea(){
  closeClientFiscalArea();const realLogo=document.querySelector(".brand img")?.src||"/app-icon.png",overlay=document.createElement("div");overlay.className="client-fiscal-overlay";
  const otherServices=[["Asesoría Laboral","people"],["Protección de Datos","shield"],["Auditoría de Cuentas","chart"],["Gestión Administrativa","admin"],["Servicios Jurídicos","legal"]];
