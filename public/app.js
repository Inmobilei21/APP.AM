...
function renderBankIbans(values=["",""]){
  const list=document.querySelector("#bankIbanList");if(!list)return;const items=values.length>=2?values:[...(values||[]),"",""].slice(0,2);
  list.innerHTML=items.map((value,index)=>`<div class="bank-iban-row"><label>IBAN ${index+1}<input type="text" data-bank-iban maxlength="34" value="${escapeHtml(value)}"></label>${index>1?'<button type="button" data-remove-iban="'+index+'" aria-label="Eliminar IBAN">×</button>':""}</div>`).join("");
...
function resetCommercialRegistry(){
  ...
  renderBankIbans(["",""]);fillMunicipalitySelect("registryProvince","registryMunicipality");fillMunicipalitySelect("bankProvince","bankMunicipality");toggleCommercialRegistry();
}
function populateCommercialRegistry(value={}){
  ...
  document.querySelector("#registryCertificateName").textContent=value.certificateName||"Ningún certificado guardado";toggleCommercialRegistry();
}
...
function setClientViewMode(viewOnly){
  ...
  document.querySelectorAll("#commercialRegistrySection button").forEach(button=>button.disabled=viewOnly);
  document.querySelector("#editClientFromView").hidden=!viewOnly;
  document.querySelector("#saveClientButton").hidden=viewOnly;
  document.querySelector("#clientModalDismiss").textContent=viewOnly?"Cerrar":"Cancelar";
}
function openClientModal(){
  ...
  document.querySelector("#saveClientButton").textContent="Guardar cliente";
...
async function openClientRecord(name,viewOnly=true){
  ...
  document.querySelector("#saveClientButton").textContent="Guardar cambios";
  document.querySelector("#formMessage").textContent="";
  showClientModal();
}
...
async function createClient(e){
  ...
  const obligations={};document.querySelectorAll("[data-tax-model]:checked").forEach(check=>{obligations[check.dataset.taxModel]=true});await saveClientMetadata({id:name,name,cif:document.querySelector("#clientCif").value.trim().toUpperCase(),personType,commercialRegistry:personType==="juridica"?collectCommercialRegistryData(registryCertificateName):null,administrators:document.querySelector("#clientAdministrators").value.trim(),phones:document.querySelector("#clientPhones").value.trim(),emails:document.querySelector("#clientEmails").value.trim(),representative:document.querySelector("#clientRepresentative").value.trim(),representativeNif:document.querySelector("#clientRepresentativeNif").value.trim().toUpperCase(),periodicity:document.querySelector("#fiscalPeriodicity").value,obligations,courtesyDaysRequired:document.querySelector("#courtesyDaysRequired").checked});
  ...
  catch(error){if(error.name!=="AbortError"){message.className="form-message error";message.textContent=error.userMessage||"No se pudo guardar el cliente. Comprueba el permiso de escritura."}}finally{button.disabled=false;button.textContent=e.target.dataset.editing?"Guardar cambios":"Guardar cliente"}
}
