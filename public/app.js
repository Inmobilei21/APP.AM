...
function addClientPerson(kind,value={}){
  const partner=kind==='partner',row=document.createElement('div');row.className='client-person-row';row.dataset.personKind=kind;
  ...
  const remove=document.createElement('button');remove.type='button';remove.className='secondary-button';remove.textContent=partner?'Eliminar socio':'Eliminar contacto';remove.onclick=()=>row.remove();row.append(remove);
  document.querySelector(partner?'#clientPartnerRows':'#clientContactRows').append(row);
}
...
function collectClientPeople(){
  ...
  return {administrationType:document.querySelector('#clientAdministrationType').value,partners:read('partner'),contacts:read('contact')};
}
...
async function saveClientMetadata(data){const d=await folderDb();await new Promise((ok,no)=>{const tx=d.transaction("clientMetadata","readwrite");tx.objectStore("clientMetadata").put(data);tx.oncomplete=()=>{d.close();ok()};tx.onerror=()=>no(tx.error)});await remoteMetadata("clients","PUT",data)}
