...
async function openExistingSignaturePicker(){
 ...
 try{let root=await getSavedHandle("signatures-folder");if(root&&await root.requestPermission({mode:"read"})!=="granted")root=null;if(!root){root=await window.showDirectoryPicker({mode:"readwrite"});await saveHandle("signatures-folder",root)}const names=[];for await(const entry of root.values())if(entry.kind==="file")names.push(entry.name);names.sort((a,b)=>a.localeCompare(b,"es",{sensitivity:"base"}));renderExistingSignatureOptions(names);search.value="";search.focus()}catch(error){if(error.name==="AbortError"){picker.hidden=true;return}list.innerHTML='<p class="existing-signature-empty">No se pudo abrir la carpeta de firmas.</p>'}
}
function renderExistingSignatureOptions(names){
 const list=document.querySelector("#existingSignatureList");list.innerHTML=names.length?names.map(name=>`<button type="button" data-existing-signature="${escapeHtml(name)}"><span class="existing-signature-file">▱</span><span title="${escapeHtml(name)}">${escapeHtml(name)}</span><b>Vincular</b></button>`).join(""):'<p class="existing-signature-empty">No hay documentos en la carpeta.</p>';
 list.querySelectorAll("[data-existing-signature]").forEach(button=>button.addEventListener("click",async()=>{const name=button.dataset.existingSignature,form=document.querySelector("#newClientForm");form.dataset.existingSignature=name;document.querySelector("#clientSignature").value="";document.querySelector("#linkedSignatureName").textContent=`Vinculada: ${name}`;document.querySelector("#existingSignaturePicker").hidden=true;const metadata=(await getAllSignatureMetadata()).find(item=>item.id===name||item.document===name);if(metadata){document.querySelector("#signatureExpiry").value=metadata.expiry||"";document.querySelector("#signaturePassword").value=metadata.password||""}}));
}
function filterExistingSignatures(event){const query=event.target.value.trim().toLocaleLowerCase("es");document.querySelectorAll("#existingSignatureList [data-existing-signature]").forEach(button=>button.hidden=!button.dataset.existingSignature.toLocaleLowerCase("es").includes(query))}
...
async function createClient(e){
  ...
  const sig=document.querySelector("#clientSignature");if(sig.files.length){let signaturesRoot=await getSavedHandle("signatures-folder");if(signaturesRoot&&await signaturesRoot.requestPermission({mode:"readwrite"})!=="granted")signaturesRoot=null;if(!signaturesRoot){signaturesRoot=await window.showDirectoryPicker({mode:"readwrite"});await saveHandle("signatures-folder",signaturesRoot)}const file=sig.files[0],savedName=`${name} - ${file.name}`;await copyNamedFile(file,signaturesRoot,savedName);await saveSignatureMetadata({id:savedName,client:name,document:savedName,password:document.querySelector("#signaturePassword").value,expiry:document.querySelector("#signatureExpiry").value})}else if(e.target.dataset.existingSignature){const existingName=e.target.dataset.existingSignature;await saveSignatureMetadata({id:existingName,client:name,document:existingName,password:document.querySelector("#signaturePassword").value,expiry:document.querySelector("#signatureExpiry").value})}
  const obligations={};document.querySelectorAll("[data-tax-model]:checked").forEach(check=>{obligations[check.dataset.taxModel]=true});await saveClientMetadata({id:name,name,cif:document.querySelector("#clientCif").value.trim().toUpperCase(),administrators:document.querySelector("#clientAdministrators").value.trim(),phones:document.querySelector("#clientPhones").value.trim(),emails:document.querySelector("#clientEmails").value.trim(),representative:document.querySelector("#clientRepresentative").value.trim(),representativeNif:document.querySelector("#clientRepresentativeNif").value.trim().toUpperCase(),periodicity:document.querySelector("#fiscalPeriodicity").value,obligations,courtesyDaysRequired:document.querySelector("#courtesyDaysRequired").checked});
...
async function loadSignatures(){
 ...
 window.signatureFiles=rows;body.innerHTML=rows.length?rows.map((r,i)=>{const status=signatureExpiryStatus(r.expiry);return `<tr data-search="${escapeHtml((r.client+" "+r.document+" "+status.label+" "+r.representative+" "+r.representativeNif).toLocaleLowerCase("es"))}"><td><strong>${escapeHtml(r.client)}</strong></td><td><button class="document-link" data-download="${i}">⇩ ${escapeHtml(r.document)}</button></td><td><span class="signature-status ${status.className}">${status.label}</span></td><td><button class="password-cell" data-password="${escapeHtml(r.password)}">${r.password?"••••••••":"—"}</button></td><td><span class="expiry ${expiryClass(r.expiry)}">${formatDate(r.expiry)}</span></td><td>${escapeHtml(r.representative||"—")}</td><td>${escapeHtml(r.representativeNif||"—")}</td></tr>`}).join(""):'<tr><td colspan="7" class="table-empty">Todavía no hay firmas digitales.</td></tr>';
