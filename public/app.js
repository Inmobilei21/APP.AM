...
function renderManagement(){
  main.innerHTML=`
    ...
    <div data-management-panel="models" hidden></div><div data-management-panel="procedures" hidden></div>
...
function openManagementSection(name){
  ...
  if(name==="procedures")renderManagementProcedures();
}
...
function renderManagementProcedures(){
  const panel=document.querySelector('[data-management-panel="procedures"]');if(!panel||panel.dataset.ready)return;panel.dataset.ready="true";
  panel.innerHTML=`<section class="management-workspace-head"><div><p class="eyebrow">GUÍAS DE TRABAJO</p><h2>Procedimientos</h2><p>Pasos de control iniciales basados en normativa española. Deben revisarse para cada expediente.</p></div><label class="management-library-search"><span>⌕</span><input id="procedureSearch" type="search" placeholder="Buscar procedimiento…"></label></section><section class="procedure-grid" id="procedureGrid"></section>`;
  document.querySelector("#procedureSearch").addEventListener("input",event=>drawManagementProcedures(event.target.value));drawManagementProcedures();
}
function drawManagementProcedures(query=""){
  const grid=document.querySelector("#procedureGrid"),q=query.trim().toLocaleLowerCase("es"),items=MANAGEMENT_PROCEDURES.filter(item=>(item.title+" "+item.category+" "+item.summary).toLocaleLowerCase("es").includes(q));
  grid.innerHTML=items.length?items.map(item=>`<article class="procedure-card"><div class="procedure-card-head"><span>${item.category}</span><b>${String(item.steps.length).padStart(2,"0")} pasos</b></div><h3>${item.title}</h3><p>${item.summary}</p><ol>${item.steps.map(step=>`<li>${step}</li>`).join("")}</ol><div class="procedure-law"><strong>Marco de referencia</strong><small>${item.law}</small></div><div class="procedure-actions"><a href="${item.source}" target="_blank" rel="noopener">Ver normativa ↗</a><button type="button" data-print-procedure="${item.id}">Imprimir</button></div></article>`).join(""):'<div class="management-library-no-results">No se encontraron procedimientos.</div>';
  grid.querySelectorAll("[data-print-procedure]").forEach(button=>button.addEventListener("click",()=>{const item=MANAGEMENT_PROCEDURES.find(value=>value.id===button.dataset.printProcedure);printManagementText(item.title,`${item.summary}\n\nPASOS DE CONTROL\n\n${item.steps.map((step,index)=>`${index+1}. ${step}`).join("\n\n")}\n\nMARCO DE REFERENCIA\n${item.law}\n\nNota: guía de control interno. Revisar la normativa vigente y las circunstancias concretas antes de ejecutar el expediente.`)}));
}
