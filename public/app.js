...
function renderClientPreview(){
  ...
  const documentRows=documents.map(item=>`<div class="client-desktop-document"><span>⌑</span><div><strong>${item[0]}</strong><small>Asesoría Fiscal y Contable &gt; ${item[2]}</small></div><time>${item[1]}</time></div>`).join("");
  ...
  const desktop=`<div class="client-desktop-shell"><header class="client-desktop-header"><img src="${realLogo}" alt="Asesoría Molinero"><span class="client-desktop-motto">Tu tranquilidad,<br>nuestro compromiso</span><nav><button class="active" type="button">⌂ Inicio</button><button type="button">▤ Mis documentos</button><button type="button">● Mensajes</button><button class="client-preview-user client-desktop-profile" type="button">♙ Mi perfil</button></nav><button class="client-desktop-logout" type="button" aria-disabled="true">↪ Cerrar sesión</button></header><section class="client-desktop-hero"><div><h1>👋 Hola, Inmobilei</h1><h2>Bienvenido a tu área de cliente</h2><p>Aquí tienes toda tu documentación, comunicaciones y gestiones con<br>Asesoría Molinero, de forma rápida, segura y siempre a tu alcance.</p></div><div class="client-desktop-hero-image"><strong>ASESORÍA<br>MOLINERO</strong><small>Personas que te acompañan</small></div></section><div class="client-desktop-content"><section class="client-desktop-services"><div class="client-desktop-title"><div><h2>Nuestros servicios</h2><p>Accede a tus servicios contratados o descubre todo lo que podemos hacer por ti.</p></div><em>Tu crecimiento también es nuestro objetivo</em></div><div class="client-desktop-services-grid">${clientDesktopServiceCard("Asesoría Fiscal y Contable","document",true)}${clientDesktopServiceCard("Asesoría Laboral","people")}${clientDesktopServiceCard("Protección de Datos","shield")}${clientDesktopServiceCard("Auditoría de Cuentas","chart")}${clientDesktopServiceCard("Gestión Administrativa","admin")}${clientDesktopServiceCard("Servicios Jurídicos","legal")}</div></section><section class="client-desktop-dashboard"><article class="client-desktop-panel client-desktop-documents"><div class="client-desktop-panel-title"><h3>▰ Últimos documentos</h3><button type="button">Ver todos →</button></div>${documentRows}</article><article class="client-desktop-panel client-desktop-messages"><div class="client-desktop-panel-title"><h3>● Mensajes recientes</h3><button type="button">Ver todos →</button></div>${messageRows}<button class="client-desktop-outline" type="button">Ir al chat →</button></article><aside><article class="client-desktop-panel client-desktop-help"><h3>➤ ¿Necesitas algo?</h3><p>Estamos aquí para ayudarte. Puedes enviarnos un mensaje, una consulta o solicitar información sobre cualquier servicio.</p><button type="button">◌ Nuevo mensaje</button></article><article class="client-desktop-contact"><h3>⌕ También puedes contactarnos</h3><p>⌕ &nbsp;953 24 12 00</p><p>✉ &nbsp;info@asesoriamolinero.es</p><p>⌖ &nbsp;C/ Ejemplo 12, 23001 Jaén</p></article></aside></section></div><footer class="client-desktop-footer"><span><img src="/app-icon.png" alt=""><strong>ASESORÍA<br>MOLINERO</strong><i>Tu tranquilidad, nuestro compromiso</i></span><small>Política de privacidad &nbsp; | &nbsp; Aviso legal &nbsp; | &nbsp; Contacto</small><em>Gracias por confiar en nosotros ♡</em></footer></div>`;
  main.innerHTML=mobile+desktop;updateProfileButtons();document.querySelectorAll(".client-preview-user").forEach(button=>button.onclick=openAccountPanel);
}
...
async function createClient(e){
  ...
  const obligations={};document.querySelectorAll("[data-tax-model]:checked").forEach(check=>{obligations[check.dataset.taxModel]=true});await saveClientMetadata({...existingClientData,...collectClientPeople(),id:name,name,cif:document.querySelector("#clientCif").value.trim().toUpperCase(),personType,commercialRegistry:personType==="juridica"?collectCommercialRegistryData(registryCertificateName):null,administrators:document.querySelector("#clientAdministrators").value.trim(),phones:document.querySelector("#clientPhones").value.trim(),emails:document.querySelector("#clientEmails").value.trim(),representative:document.querySelector("#clientRepresentative").value.trim(),representativeNif:document.querySelector("#clientRepresentativeNif").value.trim().toUpperCase(),periodicity:document.querySelector("#fiscalPeriodicity").value,active:document.querySelector("#clientActive").checked,obligations,courtesyDaysRequired:document.querySelector("#courtesyDaysRequired").checked,financialReportingFramework:document.querySelector("#financialReportingFramework").value.trim()});
