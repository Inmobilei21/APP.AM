...
function installClientAppAvailabilityToggle(){const personType=document.querySelector("#clientPersonType")?.closest("label"),active=document.querySelector("#clientActive")?.closest("label");if(!personType||!active||document.querySelector("#clientAppEnabled"))return;const field=document.createElement("label");field.className="client-active-field client-app-field";field.innerHTML='<span>Aplicación disponible</span><span class="client-active-control"><input id="clientAppEnabled" type="checkbox"><i aria-hidden="true"></i><small>Permitir acceso al portal</small></span>';active.after(field)}
...
function createWorkerChat(){
  ...
  widget.innerHTML=`
    <button class="suggestion-launcher" id="suggestionLauncher" type="button"><span aria-hidden="true">＋</span> Sugerencias</button><button class="chat-launcher" id="chatLauncher" type="button" aria-label="Abrir chat de trabajadores">
