...
async function renderClientFiscalInformation(content){
  const client=(await getAllClientMetadata()).find(item=>clientIdentity(item)===clientPreviewName)||{},contacts=client.contacts||[],partners=client.partners||[],registry=client.commercialRegistry||{},address=registry.registeredAddress||{},bank=registry.bank||{};
  const primary=contacts.find(item=>item.primary)||contacts[0]||{},models=Object.keys(client.obligations||{}).filter(key=>client.obligations[key]);
  let signature={};try{signature=(await getAllSignatureMetadata()).find(item=>item.client===clientPreviewName)||{}}catch{}
  ...
  <div class="client-fiscal-info-sections compact">
    <section><h3>Datos del cliente</h3><div class="client-fiscal-info-grid compact-grid">${clientFiscalField("Nombre o razón social",client.name||clientPreviewName,true)}${clientFiscalField("DNI / CIF",client.cif)}${clientFiscalField("Tipo de persona",client.personType==="fisica"?"Persona física":"Persona jurídica")}${clientFiscalField("Cliente activo",client.active===false?"No":"Sí")}${clientFiscalField("Aplicación disponible",client.appAccessEnabled?"Sí":"No")}</div></section>
    <section><h3>Personas físicas</h3><div class="client-fiscal-subsection"><h4>Socios</h4><div class="client-fiscal-record-head"><span>Nombre</span><span>DNI</span><span>Participación</span></div>${rows(partners,[["Nombre","name"],["DNI","dni"],["Participación","percentage"," %"]],"No hay socios registrados.")}<div class="client-fiscal-admin-type"><small>Tipo de administración</small><strong>${escapeHtml(clientFiscalEmptyValue(client.administrationType))}</strong></div></div><div class="client-fiscal-subsection"><h4>Contactos</h4><div class="client-fiscal-record-head"><span>Nombre</span><span>Teléfono</span><span>Correo</span></div>${rows(contacts,[["Nombre","name"],["Teléfono","phone"],["Correo","email"]],"No hay contactos registrados.")}</div></section>
    <section><h3>Obligaciones fiscales</h3><div class="client-fiscal-summary-line"><span><small>Periodicidad</small><strong>${client.periodicity==="mensual"?"Mensual":"Trimestral"}</strong></span><span><small>Días de cortesía</small><strong>${client.courtesyDaysRequired?"Sí":"No"}</strong></span></div><div class="client-fiscal-models">${models.length?models.map(model=>`<span>Modelo ${escapeHtml(model)}</span>`).join(""):'<em>No hay modelos fiscales seleccionados.</em>'}</div></section>
    <section><h3>Marco de información financiera aplicable</h3><div class="client-fiscal-single-value">${escapeHtml(clientFiscalEmptyValue(client.financialReportingFramework))}</div></section>
    <section><h3>Firma digital</h3><div class="client-fiscal-summary-line"><span><small>Documento</small><strong>${escapeHtml(clientFiscalEmptyValue(signature.document||signature.id))}</strong></span><span><small>Fecha de caducidad</small><strong>${escapeHtml(clientFiscalEmptyValue(signature.expiry?formatDate(signature.expiry):""))}</strong></span></div></section>
    <section><h3>Registro Mercantil</h3><div class="client-fiscal-info-grid compact-grid">${clientFiscalField("Certificado",registry.certificateName,true)}${clientFiscalField("Dirección",address.address,true)}${clientFiscalField("CP",address.postalCode)}${clientFiscalField("Provincia",window.SPAIN_LOCATIONS?.[address.province]?.name||address.province)}${clientFiscalField("Municipio",address.municipality)}${clientFiscalField("Banco",bank.name)}${clientFiscalField("Titular",bank.holder)}${clientFiscalField("IBAN",(bank.ibans||[]).join(" · "),true)}</div></section>
  </div>`;
}
function openClientFiscalArea(){
  closeClientFiscalArea();const realLogo=document.querySelector(".brand img")?.src||"/app-icon.png",overlay=document.createElement("div");overlay.className="client-fiscal-overlay";
