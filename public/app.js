...
function openClientFiscalArea(){
  ...
  overlay.querySelector(".client-fiscal-back").addEventListener("click",closeClientFiscalArea);overlay.querySelectorAll("[data-fiscal-tab]").forEach(button=>button.addEventListener("click",()=>render(button.dataset.fiscalTab)));overlay.querySelectorAll("[data-client-unavailable]").forEach(button=>button.addEventListener("click",()=>openClientUnavailable(button.dataset.clientUnavailable)));render("documents");
}
...
function renderClientPreview(){
  ...
  main.querySelectorAll(".client-service-card.contracted,.client-desktop-service.contracted>button").forEach(button=>{button.removeAttribute("aria-disabled");button.addEventListener("click",openClientFiscalArea)});
...
async function renderClientFiscalInformation(content){
  ...
  const rows=(items,fields,empty)=>items.length?items.map(item=>`<div class="client-fiscal-record-row">${fields.map(([label,key,suffix])=>`<span><small>${escapeHtml(label)}</small><strong>${escapeHtml(clientFiscalEmptyValue(item[key]))}${item[key]!==undefined&&item[key]!==null&&item[key]!==""&&suffix?suffix:""}</strong></span>`).join("")}</div>`).join(""):`<p class="client-fiscal-no-data">${empty}</p>`;
  const protectedValue=value=>value?"••••••••":"—";
  content.innerHTML=`<div class="client-fiscal-section-title"><div><h2>Información general</h2><p>Información registrada en tu ficha de cliente. Estos datos son solo de consulta.</p></div><span class="client-fiscal-readonly">Solo lectura</span></div>
  <div class="client-fiscal-info-sections compact bento">
    <section class="client-info-card client-info-identity"><h3>Datos del cliente</h3><div class="client-fiscal-info-grid compact-grid">${clientFiscalField("Nombre o razón social",client.name||clientPreviewName,true)}${clientFiscalField("DNI / CIF",client.cif)}${clientFiscalField("Tipo de persona",client.personType==="fisica"?"Persona física":"Persona jurídica")}${clientFiscalField("Cliente activo",client.active===false?"No":"Sí")}${clientFiscalField("Aplicación disponible",client.appAccessEnabled?"Sí":"No")}</div></section>
    <section class="client-info-card client-info-people"><h3>Personas físicas</h3><div class="client-fiscal-subsection"><h4>Socios</h4><div class="client-fiscal-record-head"><span>Nombre</span><span>DNI</span><span>Participación</span></div>${rows(partners,[["Nombre","name"],["DNI","dni"],["Participación","percentage"," %"]],"No hay socios registrados.")}<div class="client-fiscal-admin-type"><small>Administración</small><strong>${escapeHtml(clientFiscalEmptyValue(client.administrationType))}</strong></div></div><div class="client-fiscal-subsection"><h4>Contactos</h4><div class="client-fiscal-record-head"><span>Nombre</span><span>Teléfono</span><span>Correo</span></div>${rows(contacts,[["Nombre","name"],["Teléfono","phone"],["Correo","email"]],"No hay contactos registrados.")}</div></section>
    <section class="client-info-card client-info-tax"><h3>Obligaciones fiscales</h3><div class="client-fiscal-summary-line"><span><small>Periodicidad</small><strong>${client.periodicity==="mensual"?"Mensual":"Trimestral"}</strong></span><span><small>Días de cortesía</small><strong>${client.courtesyDaysRequired?"Sí":"No"}</strong></span></div><div class="client-fiscal-models">${models.length?models.map(model=>`<span>Modelo ${escapeHtml(model)}</span>`).join(""):'<em>No hay modelos seleccionados.</em>'}</div></section>
    <section class="client-info-card client-info-framework"><h3>Marco financiero aplicable</h3><div class="client-fiscal-single-value">${escapeHtml(clientFiscalEmptyValue(client.financialReportingFramework))}</div></section>
    <section class="client-info-card client-info-signature"><h3>Firma digital</h3><div class="client-fiscal-summary-line"><span><small>Documento</small><strong>${escapeHtml(clientFiscalEmptyValue(signature.document||signature.id))}</strong></span><span><small>Caducidad</small><strong>${escapeHtml(clientFiscalEmptyValue(signature.expiry?formatDate(signature.expiry):""))}</strong></span></div></section>
    <section class="client-info-card client-info-registry"><h3>Registro Mercantil</h3><div class="client-registry-tabs" role="tablist">${[["domicilio","Domicilio"],["bancos","Bancos"],["acceso","Acceso"],["seguridad","Seguridad"],["observaciones","Observaciones"]].map(([key,label],index)=>`<button type="button" role="tab" data-client-registry-tab="${key}" aria-selected="${index===0}">${label}</button>`).join("")}</div>
      <div class="client-registry-panels">
        <section data-client-registry-panel="domicilio"><div class="client-fiscal-info-grid compact-grid">${clientFiscalField("Certificado",registry.certificateName,true)}${clientFiscalField("Dirección",address.address,true)}${clientFiscalField("CP",address.postalCode)}${clientFiscalField("Provincia",window.SPAIN_LOCATIONS?.[address.province]?.name||address.province)}${clientFiscalField("Municipio",address.municipality)}</div></section>
        <section data-client-registry-panel="bancos" hidden><div class="client-fiscal-info-grid compact-grid">${clientFiscalField("Titular",bank.holder)}${clientFiscalField("CIF",bank.cif)}${clientFiscalField("BIC",bank.bic)}${clientFiscalField("CCC",bank.ccc)}${clientFiscalField("IBAN",(bank.ibans||[]).join(" · "),true)}${clientFiscalField("Banco",bank.name)}${clientFiscalField("Dirección",bank.address,true)}${clientFiscalField("CP",bank.postalCode)}${clientFiscalField("Provincia",window.SPAIN_LOCATIONS?.[bank.province]?.name||bank.province)}${clientFiscalField("Municipio",bank.municipality)}</div></section>
        <section data-client-registry-panel="acceso" hidden><div class="client-fiscal-info-grid compact-grid">${clientFiscalField("Usuario",access.username,true)}${clientFiscalField("Contraseña",protectedValue(access.password),true)}</div></section>
        <section data-client-registry-panel="seguridad" hidden><div class="client-fiscal-info-grid compact-grid">${clientFiscalField("Usuario",security.username,true)}${clientFiscalField("Contraseña",protectedValue(security.password),true)}</div></section>
        <section data-client-registry-panel="observaciones" hidden><div class="client-fiscal-single-value client-fiscal-observations">${escapeHtml(clientFiscalEmptyValue(registry.observations))}</div></section>
      </div>
    </section>
  </div>`;
