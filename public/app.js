...
function renderClientPreview(){
  ...
  main.innerHTML=mobile+desktop;updateProfileButtons();document.querySelectorAll(".client-preview-user").forEach(button=>button.onclick=openAccountPanel);document.querySelectorAll("[data-client-unavailable]").forEach(card=>card.addEventListener("click",()=>openClientUnavailable(card.dataset.clientUnavailable)));
}
